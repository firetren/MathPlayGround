    ...
    tabContainer.addEventListener('click', (e) => {
      ...
      if (button) {
        ...
        tabContainer.querySelectorAll('[data-category]').forEach(btn => {
          btn.classList.remove('scale-105', 'brightness-110', 'ring-4', 'ring-[#2b70e4]/40', 'opacity-100', 'z-10');
          btn.classList.add('opacity-80', 'scale-100');
        });
        button.classList.add('scale-105', 'brightness-110', 'ring-4', 'ring-[#2b70e4]/40', 'opacity-100', 'z-10');
        button.classList.remove('opacity-80', 'scale-100');
...
function renderActiveGrid() {
          ...
          newEmptyBtn.addEventListener('click', () => {
            ...
            if (tabContainer) {
              tabContainer.querySelectorAll('[data-category]').forEach(btn => {
                ...
                if (cat === 'All') {
                  btn.classList.add('scale-105', 'brightness-110', 'ring-4', 'ring-[#2b70e4]/40', 'opacity-100', 'z-10');
                  btn.classList.remove('opacity-80', 'scale-100');
                } else {
                  btn.classList.remove('scale-105', 'brightness-110', 'ring-4', 'ring-[#2b70e4]/40', 'opacity-100', 'z-10');
                  btn.classList.add('opacity-80', 'scale-100');
                }
  ...
  filtered.forEach(game => {
    ...
    card.className = "group relative cursor-pointer flex flex-col items-center w-full transition-all duration-300 transform hover:scale-[1.04]";
      ...
      <div class="relative w-full aspect-square overflow-hidden rounded-2xl bg-slate-100 shadow-sm border border-slate-200/60 shrink-0">
        ...
        <div class="absolute top-2 right-2 z-10 flex gap-1 bg-white/75 backdrop-blur-sm rounded-lg p-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-auto">
          <button
            ...
            class="p-1 rounded-md text-slate-600 hover:text-orange-500 hover:bg-slate-100 cursor-pointer transition-colors"
          ...
          <button
            ...
            class="p-1 rounded-md cursor-pointer transition-colors ${
              isFav 
                ...
                : 'text-slate-600 hover:text-pink-500 hover:bg-slate-100'
            }"
            title="${isFav ? 'Remove from Favorites' : 'Add to Favorites'}"
          >
            <svg class="w-3.5 h-3.5 ${isFav ? 'fill-current' : ''}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>
          </button>

          ${game.custom ? `
            <button
              data-action="delete-custom"
              class="p-1 rounded-md text-rose-500 hover:bg-rose-50 cursor-pointer transition-colors"
              title="Delete custom game"
            >
              <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18m-2 0v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
            </button>
          ` : ''}
        </div>
        ...
        ` : `
          ...
          <div class="w-full h-full bg-gradient-to-br ${gradientColor} opacity-90 flex items-center justify-center relative">
            <div class="absolute inset-0 bg-slate-950/10"></div>
            <div class="transform group-hover:scale-110 transition-transform duration-300">
              ${getIconSVG(game.icon, 'w-8 h-8 text-white drop-shadow-sm')}
            </div>
          </div>
        `}
      </div>
...
function launchGame(game) {
  ...
  // Hide main header to mimic Pic 2 exactly
...
function setupArenaView(game) {
  ...
  // Dynamic Pic 2 header and banner updates
  const hdLogo = document.getElementById('arena-header-headline-logo');
  if (hdLogo) {
    hdLogo.textContent = game.title.toUpperCase() + '...';
  }
  const badgeLetter = document.getElementById('arena-banner-badge-letter');
  if (badgeLetter) {
    badgeLetter.textContent = game.title.charAt(0).toUpperCase();
  }
  const statusPrompt = document.getElementById('arena-banner-instruction-prompt');
  if (statusPrompt) {
    statusPrompt.textContent = game.description ? game.description.toUpperCase() : 'REACH THE FINISH LINE SAFELY!';
  }
  const footerQuote = document.getElementById('arena-game-footer-quote');
  if (footerQuote) {
    footerQuote.textContent = game.instructions ? game.instructions : 'At each level, the goal is to reach the portal safely. Can you pass all obstacles?';
  }
...
function getIconSVG(name, cls) {
  switch (name) {
    ...
    case 'Bomb':
      return `<svg class="${cls}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="13" r="8"/><path d="M18.5 5.5 21 3"/><path d="M14.5 9.5 15.5 8.5"/><path d="M19 8.5h2"/><path d="M16 4.5v2"/></svg>`;
    default:
...
// Global window registrations for Pic 2 workflow
window.xyzCloseSelectedGame = closeSelectedGame;
window.handleGradeClickFromArena = (grade) => {
  closeSelectedGame();
  
  const query = (grade === "pre-k") ? "pre-k" :
